	...
	<div class="content">
		<table>
			<tr>
				...
				@else
				<td class="" style="width: 150px;">
					<img src="{{'data:image/png;base64,' . base64_encode(file_get_contents(@public_path('logos/logonfenotas.png')))}}" width="100px;">
				</td>
				@endif
		...
		<tbody>
			...
			<tr>
				...
				<th class="b-top">
					...
					{{-- LOTE E VENCIMENTO --}}
@if(!empty($i->produto->lote))
    | Lote: {{ $i->produto->lote }}

    @if(!empty($i->produto->vencimento))
        , Vencimento: {{ \Carbon\Carbon::parse($i->produto->vencimento)->format('d/m/Y') }}
    @endif
@endif 

{{-- IMEI --}}
@if(!empty($i->produto->imei))
    | IMEI: {{ $i->produto->imei }}
@endif
				</th class="b-top">
				<th class="b-top">
		...
		<tr>
			<td class="" style="width: 700px;">
				Forma de pagamento: <strong> 
					...
					@if($item->getFormaPagamento($item->empresa_id) != null)
					- <span style="color: #8950FC">{{ $item->getFormaPagamento($item->empresa_id)->infos }}</span>
					@endif
				</strong>
			</td>
		</tr>
