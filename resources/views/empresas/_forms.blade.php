...
<div class="row g-3">
    ...
    <div class="col-md-4">
        ...
        ->options(($empresa != null && $empresa->cidade) ? [$empresa->cidade_id => $empresa->cidade->info] : [])->required()
        !!}
    </div>
    @else
    <div class="col-md-4">
    ...
    @endif
<div class="col-md-3">
{!! Form::select('contador_id', 'Contador (Opcional)', [null => 'Selecione um contador'] + $contadores->toArray())
    ->value($empresa->contador_id ?? null)
            ...
            <div class="form-group validated col-sm-4 col-lg-4">
                ...
                <div class="">
                    <select id="perfil-select" class="form-select" name="perfil_id">
                        ...
                        @foreach ($perfis as $p)
                        <option 
                        @isset($empresa) @if($empresa->perfil_id == $p->id) selected @endif @endisset value="{{ $p }}">
                            {{ $p->nome }}
                        </option>
                        @endforeach
                    </select>
            ...
            <div class="col-12 col-form-label mt-2">
                ...
                <div class="checkbox-inline" style="margin-top: 10px;">
                    ...
                    <label class="checkbox checkbox-info check-sub">
                        <input class="{{str_replace(' ', '', $m['titulo'])}} {{$link}}" type="checkbox" name="{{ $s['rota'] }}" @isset($empresa) @if(in_array($s['rota'], $permissoesAtivas)) checked @endif @endisset>
                        <span></span>{{ $s['nome'] }}
                    </label>
                    @endforeach
