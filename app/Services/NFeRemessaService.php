...
class NFeRemessaService
{
	...
	public function gerarNFe($venda)
	{
		...
		$config = ConfigNota::where('empresa_id', $this->empresa_id)
    		...
    		if (!empty($venda->data_retroativa)) {
    			$dataEmissao = new \DateTime(
    				$venda->data_retroativa,
			...
			$stdIde->dhEmi    = $dataEmissao->format('Y-m-d\TH:i:sP');
		    $stdIde->dhSaiEnt = $dataEmissao->format('Y-m-d\TH:i:sP');
	...
	public function transmitir($signXml, $chave)
	{
		try {
			/**
			 * ENVIO SÍNCRONO (1 NF-e)
			 */
			$resp = $this->tools->sefazEnviaLote([$signXml], 1, 1);
			// terceiro parâmetro = 1 → síncrono
